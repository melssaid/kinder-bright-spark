    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        <div className="flex items-center justify-between text-sm">
          <span>{t("nutrition.dailyCalories")}</span>
        ...
        {child.nutrition.map((meal, i) => (
          <div key={i} className="p-2 rounded-lg border bg-muted/20 space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground ms-auto">{meal.calories} kcal</span>
            </div>
            <p className="text-xs text-muted-foreground">{meal.items.join(", ")}</p>
            <div className="grid grid-cols-3 gap-2 text-[10px]">
              <div><span className="text-muted-foreground">{t("nutrition.protein")}</span><Progress value={meal.protein * 3} className="h-1 mt-0.5" /></div>
              <div><span className="text-muted-foreground">{t("nutrition.carbs")}</span><Progress value={meal.carbs} className="h-1 mt-0.5" /></div>
              <div><span className="text-muted-foreground">{t("nutrition.vitamins")}</span><Progress value={meal.vitamins} className="h-1 mt-0.5" /></div>
            </div>
          </div>
        ))}
      </CardContent>
